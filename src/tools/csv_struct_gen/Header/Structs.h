...
namespace Excel
{
  ...
  /* 33331 */
  struct Item {
    ItemTextStruct Text;            // Text fields - typically 24 bytes
    uint64_t ModelId;               // 8 bytes
    uint64_t SubModelId;            // 8 bytes
    uint32_t StackMax;              // 4 bytes
    uint32_t CategoryArg;           // 4 bytes
    uint32_t PriceMin;              // 4 bytes
    uint32_t Price;                 // 4 bytes
    uint32_t PriceMax;              // 4 bytes
    uint32_t SortId;                // 4 bytes
    int32_t RepairItem;             // 4 bytes
    int32_t MirageItem;             // 4 bytes
    uint16_t Icon;                  // 2 bytes
    uint16_t Level;                 // 2 bytes
    uint16_t Trait;                 // 2 bytes
    uint16_t Damage;                // 2 bytes
    uint16_t MagicDamage;           // 2 bytes
    uint16_t AttackInterval;        // 2 bytes
    uint16_t ShieldRate;            // 2 bytes
    uint16_t ShieldBlockRate;       // 2 bytes
    uint16_t Defense;               // 2 bytes
    uint16_t MagicDefense;          // 2 bytes
    uint16_t RecastTime;            // 2 bytes
    uint16_t Action;                // 2 bytes
    uint16_t Salvage;               // 2 bytes
    int16_t BonusValue[ 7 ];        // 14 bytes
    uint8_t EquipLevel;             // 1 byte
    uint8_t EquipPvPRank;           // 1 byte
    uint8_t Category;               // 1 byte
    uint8_t UICategory;             // 1 byte
    uint8_t SearchCategory;         // 1 byte
    uint8_t Rarity;                 // 1 byte
    uint8_t MaterializeType;        // 1 byte
    uint8_t MateriaSocket;          // 1 byte
    uint8_t Slot;                   // 1 byte
    uint8_t BonusType[ 6 ];         // 6 bytes
    uint8_t SpecialBonus;           // 1 byte
    uint8_t SpecialBonusType[ 6 ];  // 6 bytes
    uint8_t Repair;                 // 1 byte
    uint8_t Class;                  // 1 byte
    uint8_t AttackType;             // 1 byte
    uint8_t AttackRange;            // 1 byte
    uint8_t CastTime;               // 1 byte
    uint8_t CondClassJob;           // 1 byte
    uint8_t Series;                 // 1 byte
    uint8_t SellPriceParam;         // 1 byte
    uint8_t CondGrandCompany;       // 1 byte
    uint8_t Role;                   // 1 byte
    int8_t SpecialBonusValue[ 6 ];  // 6 bytes
    bool ConditionRace[ 5 ];        // 5 bytes
    uint8_t DisablePassedOthers : 1;// Bit fields
    uint8_t OnlyOne : 1;
